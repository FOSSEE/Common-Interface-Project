def MATMUL(outroot, attribid, ordering, geometry, parameters):
    func_name = 'MATMUL'
    data_type = ['matmul_m','matzmul_m','matmul_i32','matmul_i16','matmul_i8','matmul_ui32','matmul_ui16','matmul_ui8']
    overflow = ['n', 's', 'e']
    ...
    if para2==1:
        if para1==1 or para1==2:
            simulation_func_name = data_type[para1-1]
        else:
            simulation_func_name = data_type[para1-1] + overflow[para3]
    elif para2==2:
        if para3==2:
            if para1!=2:
